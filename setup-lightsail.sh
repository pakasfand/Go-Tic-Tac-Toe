...
# Pull Docker image from Docker Hub instead of building
echo "Pulling Docker image from Docker Hub..."
if docker pull pakasfand/go-tic-tac-toe:latest; then
    docker tag pakasfand/go-tic-tac-toe:latest go-tic-tac-toe:latest
    echo "Successfully pulled Docker image from Docker Hub"
else
    echo "ERROR: Failed to pull Docker image from Docker Hub"
    echo "Please check your internet connection and Docker Hub access"
    echo "Image: pakasfand/go-tic-tac-toe:latest"
    exit 1
fi
